...
function App() {
  const { connectToDevices, disconnectToDevices } = useApplication(); // ВБерём методы объекта Application
  const [disabled, setDisabled] = useState<boolean>(true); // Переключение кнопок
  const [inputData, setInputData] = useState<ArraySensorDataInterface>({  // для графиков
    axc: { x: [], y: [], z: [] },
    vel: { x: [], y: [], z: [] },
    ang: { x: [], y: [], z: [] },
    counter: [0],
  });
  const [csv, setCsv] = useState([  // Для .csv
    {
      1: "0",
      2: "0",
      3: "0",
  ...
  const buttonLogic: any = [ // Для изменения цвета. Если убрать any начнётся говнофонтан из ошибок типов
